...
namespace Ajy
{
	namespace avltree1
	{
		...
		struct avltree_node
		{
			...
			{
				if (node->parent->parent == node && node->balance_factor == -2)
					node = node->right;
		...
		{
			init();
			copy_all<false>(_impl.header, other.root());
		...
		typename avl_tree<params_t>::iterator avl_tree<params_t>::find(key_type const& key)
		{
			...
			return (!node || get_key_comp()(key, get_key_t()(node->value))) ? end() : iterator(node);
		}
		...
		{
			link_type node = lower_bound_(key);
			return (!node || get_key_comp()(key, get_key_t()(node->value))) ? end() : const_iterator(node);
		}
		...
		{
			link_type node = lower_bound_(key);
			if (!node || get_key_comp()(key, get_key_t()(node->value)))
			{
		...
		void avl_tree<params_t>::leftright_rotate(link_type rotate_node)
		{
			...
			*/
			...
			if (rotate_node->left)   // do we have f?
				rotate_node->left->parent = rotate_node;
			if (lson->right)    // do we have e?
		...
		typename avl_tree<params_t>::link_type avl_tree<params_t>::erase_rebalance(link_type const fixup_node)
		{
			...
			if (!y->left)     // fixup_node has at most one non-null child. y == fixup_node.
				x = y->right;     // x might be null.
			else
				if (!y->right)  // fixup_node has exactly one non-null child. y == fixup_node.
			...
			{                        // y == fixup_node    --> fixup_node has only one child, or none
				x_parent = y->parent;
				if (x)        // if fixup_node has at least one child
				...
				{ // need to fix the nil?
					if (!fixup_node->right)           // fixup_node->left must be null also
				...
				{ // if fixup_node is rightmost, fixup_node don't have right
					if (!fixup_node->left)         // fixup_node->right must be null also
