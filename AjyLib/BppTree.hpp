...
namespace Ajy
{
	...
	// sizeof(key)巨大的情况下, 内存占用会偏大, 并且性能下降
	// 遍历速度任何条件下都很快!比标准库map/set快得多!
	// 有map / set / multimap / multiset实现
	// stl style
	// MSVC下natvis友好调试视图支持


	template<class config_t> class bpp_tree;
	...
	using bpptree_set = bpp_tree<bpptree_set_config_t<value_t, comparator_t, allocator_t, std::true_type>>;
	template<class value_t, class comparator_t = std::less<value_t>, class allocator_t = std::allocator<value_t>>
	using bpptree_multiset = bpp_tree<bpptree_set_config_t<value_t, comparator_t, allocator_t, std::false_type>>;
	...
	using bpptree_map = bpp_tree<bpptree_map_config_t<key_t, value_t, comparator_t, allocator_t, std::true_type>>;
	template<class key_t, class value_t, class comparator_t = std::less<key_t>, class allocator_t = std::allocator<std::pair<key_t const, value_t>>>
	using bpptree_multimap = bpp_tree<bpptree_map_config_t<key_t, value_t, comparator_t, allocator_t, std::false_type>>;


	namespace bpp_tree_detail
	{
		...
		template<class iterator_t, class in_value_t, class tag_t>
		void construct_one(iterator_t where, in_value_t&& value, tag_t)
		{
		...
		template<class iterator_t>
		void destroy_range(iterator_t destroy_begin, iterator_t destroy_end, move_trivial_tag)
		{
		}
		template<class iterator_t>
		void destroy_range(iterator_t destroy_begin, iterator_t destroy_end, move_assign_tag)
		{
		...
		void move_forward(iterator_from_t move_begin, iterator_from_t move_end, iterator_to_t to_begin, move_trivial_tag)
		{
			std::ptrdiff_t count = move_end - move_begin;
			std::memmove(&*to_begin, &*move_begin, count * sizeof(*move_begin));
		}
		template<class iterator_from_t, class iterator_to_t>
		void move_forward(iterator_from_t move_begin, iterator_from_t move_end, iterator_to_t to_begin, move_assign_tag)
		{
			std::move(move_begin, move_end, to_begin);
		}

		template<class iterator_from_t, class iterator_to_t>
		void move_backward(iterator_from_t move_begin, iterator_from_t move_end, iterator_to_t to_begin, move_trivial_tag)
		{
			std::ptrdiff_t count = move_end - move_begin;
			std::memmove(&*to_begin, &*move_begin, count * sizeof(*move_begin));
		}
		template<class iterator_from_t, class iterator_to_t>
		void move_backward(iterator_from_t move_begin, iterator_from_t move_end, iterator_to_t to_begin, move_assign_tag)
		{
			std::move_backward(move_begin, move_end, to_begin + (move_end - move_begin));
		}

		template<class iterator_from_t, class iterator_to_t>
		void move_construct(iterator_from_t move_begin, iterator_from_t move_end, iterator_to_t to_begin, move_trivial_tag)
		{
			std::ptrdiff_t count = move_end - move_begin;
			std::memmove(&*to_begin, &*move_begin, count * sizeof(*move_begin));
		}
		template<class iterator_from_t, class iterator_to_t>
		void move_construct(iterator_from_t move_begin, iterator_from_t move_end, iterator_to_t to_begin, move_assign_tag)
		{
			std::uninitialized_copy(std::make_move_iterator(move_begin), std::make_move_iterator(move_end), to_begin);
		}

		template<class iterator_t>
		void move_next_to_and_construct(iterator_t move_begin, iterator_t move_end, iterator_t to_begin, move_trivial_tag)
		{
			std::ptrdiff_t count = move_end - move_begin;
			std::memmove(&*to_begin, &*move_begin, count * sizeof(*move_begin));
		}
		template<class iterator_t>
		void move_next_to_and_construct(iterator_t move_begin, iterator_t move_end, iterator_t to_begin, move_assign_tag)
		{
			typedef typename std::iterator_traits<iterator_t>::value_type iterator_value_t;
		...
		template<class iterator_from_t, class iterator_to_t>
		void move_and_destroy(iterator_from_t move_begin, iterator_from_t move_end, iterator_to_t to_begin, move_trivial_tag)
		{
			std::ptrdiff_t count = move_end - move_begin;
			std::memmove(&*to_begin, &*move_begin, count * sizeof(*move_begin));
		}
		template<class iterator_from_t, class iterator_to_t>
		void move_and_destroy(iterator_from_t move_begin, iterator_from_t move_end, iterator_to_t to_begin, move_assign_tag)
		{
		...
		template<class iterator_t, class in_value_t>
		void move_next_and_insert_one(iterator_t move_begin, iterator_t move_end, in_value_t&& value, move_trivial_tag)
		{
			std::ptrdiff_t count = move_end - move_begin;
		...
		template<class iterator_t, class in_value_t>
		void move_next_and_insert_one(iterator_t move_begin, iterator_t move_end, in_value_t&& value, move_assign_tag)
		{
		...
		template<class iterator_t>
		void move_prev_and_destroy_one(iterator_t move_begin, iterator_t move_end, move_trivial_tag)
		{
			std::ptrdiff_t count = move_end - move_begin;
			std::memmove(&*(move_begin - 1), &*move_begin, count * sizeof(*move_begin));
		}
		template<class iterator_t>
		void move_prev_and_destroy_one(iterator_t move_begin, iterator_t move_end, move_assign_tag)
		{
	...
	protected:
		...
		struct status_select_t<std::false_type, unused_t>
		{
			status_select_t()
			{
			}
		...
		template<class, class>
		struct status_control_select_t
		{
		...
		template<class unused_t>
		struct status_control_select_t<std::false_type, unused_t>
		{
		...
		struct root_node_t : public node_t, public key_compare, public node_allocator_t, public status_t
		{
			...
			root_node_t(any_key_compare&& comp, any_allocator_t&& alloc)
		...
		struct result_t
		{
			...
			explicit result_t(result_flags_t f = btree_ok)
				: flags(result_flags_t(f & ~btree_update_lastkey))
				, last_key()
			{
			}
			...
			result_t(result_flags_t f, in_key_t&& k)
				: flags(result_flags_t(f | btree_update_lastkey))
				, last_key(std::forward<in_key_t>(k))
			{
			}
			~result_t()
			{
	...
	public:
		...
		public:
			...
			typedef typename bpp_tree::value_type value_type;
			typedef typename bpp_tree::difference_type difference_type;
			typedef typename bpp_tree::reference reference;
			typedef typename bpp_tree::pointer pointer;
		public:
			iterator(node_t* in_node, size_type in_where)
				: node(in_node)
				, where(in_where)
			{
			}
			iterator(pair_pos_t pos, bpp_tree* self)
				: node(pos.first == nullptr ? static_cast<node_t*>(&self->root_) : static_cast<node_t*>(pos.first)), where(pos.second)
			{
			}
			iterator(iterator const&) = default;
			iterator& operator += (difference_type diff)
			{
				bpp_tree::advance_step_(node, where, diff);
				return *this;
			}
			iterator &operator -= (difference_type diff)
			{
			...
			{
				iterator ret = *this;
				bpp_tree::advance_step_(ret.node, ret.where, diff);
				return ret;
			}
			iterator operator - (difference_type diff) const
			{
			...
			{
				return static_cast<difference_type>(bpp_tree::calculate_rank_(node, where)) - static_cast<difference_type>(bpp_tree::calculate_rank_(other.node, other.where));
			}
			iterator& operator++()
			{
				bpp_tree::advance_next_(node, where);
				return *this;
			}
			iterator& operator--()
			{
				bpp_tree::advance_prev_(node, where);
				return *this;
			}
		...
		public:
			...
			typedef typename bpp_tree::difference_type difference_type;
			typedef typename bpp_tree::reference reference;
			typedef typename bpp_tree::const_reference const_reference;
			typedef typename bpp_tree::pointer pointer;
			typedef typename bpp_tree::const_pointer const_pointer;
		public:
			const_iterator(node_t* in_node, size_type in_where)
				: node(in_node)
				, where(in_where)
			{
			}
			const_iterator(pair_pos_t pos, bpp_tree const* self)
				: node(pos.first == nullptr ? self->root_.parent->parent : pos.first), where(pos.second)
			{
			}
		...
		public:
			...
			typedef typename bpp_tree::pointer pointer;
		public:
			reverse_iterator(node_t* in_node, size_type in_where)
				: node(in_node)
				, where(in_where)
			{
			}
			reverse_iterator(pair_pos_t pos, bpp_tree* self)
				: node(pos.first == nullptr ? static_cast<node_t*>(&self->root_) : static_cast<node_t*>(pos.first)), where(pos.second)
			{
			}
			...
			{
				return static_cast<difference_type>(bpp_tree::calculate_rank_(other.node, other.where)) - static_cast<difference_type>(bpp_tree::calculate_rank_(node, where));
			}
		...
		public:
			...
			typedef typename bpp_tree::const_pointer const_pointer;
		public:
			const_reverse_iterator(node_t* in_node, size_type in_where)
				: node(in_node)
				, where(in_where)
			{
			}
			const_reverse_iterator(pair_pos_t pos, bpp_tree const* self)
				: node(pos.first == nullptr ? self->root_.parent->parent : pos.first), where(pos.second)
			{
			}
	...
	public:
		...
		//empty
		explicit bpp_tree(key_compare const& comp, allocator_type const& alloc = allocator_type())
			: root_(comp, alloc)
		{
		}
		//empty
		explicit bpp_tree(allocator_type const& alloc)
			: root_(key_compare(), alloc)
		{
		}
		//range
		template <class iterator_t>
		bpp_tree(iterator_t begin, iterator_t end, key_compare const& comp = key_compare(), allocator_type const& alloc = allocator_type())
			: root_(comp, alloc)
		{
			insert(begin, end);
		}
		//range
		template <class iterator_t>
		bpp_tree(iterator_t begin, iterator_t end, allocator_type const& alloc)
			: root_(key_compare(), alloc)
		{
			insert(begin, end);
		}
		//copy
		bpp_tree(bpp_tree const& other)
			: root_(other.get_comparator_(), other.get_node_allocator_())
		{
			insert(other.begin(), other.end());
		}
		//copy
		bpp_tree(bpp_tree const& other, allocator_type const& alloc)
			: root_(other.get_comparator_(), alloc)
		{
			insert(other.begin(), other.end());
		}
		//move
		bpp_tree(bpp_tree&& other)
			: root_(key_compare(), node_allocator_t())
		{
			swap(other);
		}
		//move
		bpp_tree(bpp_tree&& other, allocator_type const& alloc)
			: root_(key_compare(), alloc)
		{
			insert(std::make_move_iterator(other.begin()), std::make_move_iterator(other.end()));
		}
		//initializer list
		bpp_tree(std::initializer_list<value_type> il, key_compare const& comp = key_compare(), allocator_type const& alloc = allocator_type())
			: bpp_tree(il.begin(), il.end(), comp, alloc)
		{
		}
		//initializer list
		bpp_tree(std::initializer_list<value_type> il, allocator_type const& alloc)
			: bpp_tree(il.begin(), il.end(), key_compare(), alloc)
		{
		}
		//destructor
		~bpp_tree()
		...
		//copy
		bpp_tree& operator = (bpp_tree const& other)
		{
			if (this == &other)
			{
				return *this;
		...
		//move
		bpp_tree& operator = (bpp_tree&& other)
		{
			if (this == &other)
			{
				return *this;
		...
		//initializer list
		bpp_tree& operator = (std::initializer_list<value_type> il)
		{
			clear();
		...
		template<class in_key_t, class = typename std::enable_if<std::is_convertible<in_key_t, key_type>::value && config_t::unique_type::value && !std::is_same<key_type, storage_type>::value, void>::type>
		mapped_type& at(in_key_t&& key)
		{
			pair_pos_t pos = lower_bound_(key);
			if (pos.first == nullptr || pos.second >= pos.first->bound() || get_comparator_()(key, get_key_t()(pos.first->item[pos.second])))
			{
				throw std::out_of_range("b_plus_tree out of range");
			}
			return pos.first->item[pos.second].second;
		}

		template<class in_key_t, class = typename std::enable_if<std::is_convertible<in_key_t, key_type>::value && config_t::unique_type::value && !std::is_same<key_type, storage_type>::value, void>::type>
		mapped_type const& at(in_key_t&& key) const
		{
			pair_pos_t pos = lower_bound_(key);
			if (pos.first == nullptr || pos.second >= pos.first->bound() || get_comparator_()(key, get_key_t()(pos.first->item[pos.second])))
			{
		...
		iterator erase(const_iterator erase_begin, const_iterator erase_end)
		{
			...
			else
			{
				...
				{
					erase(indexAt(--pos_end));
				}
				return indexAt(pos_begin);
			}
		}
		...
		size_type count(key_type const& min, key_type const& max) const
		{
			...
			pair_cici_t range = bpp_tree::range(min, max);
			return std::distance(range.first, range.second);
		}
		...
		pair_ii_t slice(difference_type slice_begin = 0, difference_type slice_end = 0)
		{
			...
			return pair_ii_t(indexAt(slice_begin), indexAt(slice_end));
		}
		//reverse index when index < 0
		pair_cici_t slice(difference_type slice_begin = 0, difference_type slice_end = 0) const
		{
			...
			return pair_cici_t(indexAt(slice_begin), indexAt(slice_end));
		}
		...
		iterator indexAt(size_type index)
		{
			return iterator(access_index_(root_.parent, index), this);
		}
		//if(index >= size) return end
		const_iterator indexAt(size_type index) const
		{
	...
	protected:
		...
		template<class T> 
		void adjust_pointer_(T*& ptr, difference_type offset)
		{
			ptr = reinterpret_cast<T*>(reinterpret_cast<uint8*>(ptr) + offset);
		}
		...
		void dealloc_node_(in_node_t* node)
		{
		...
		size_type lower_bound_(node_type* node, key_type const& key) const
		{
		...
		template<typename node_type>
		size_type upper_bound_(node_type* node, key_type const& key) const
		{
			if (std::is_scalar<key_type>::value && size_type(node_type::max) < size_type(binary_search_limit))
			{
				return std::find_if(node->item, node->item + node->bound(), [&](typename node_type::item_type const& item)->bool
		...
		template<class iterator_t, class in_value_t>
		static void construct_one_(iterator_t where, in_value_t&& value)
		{
			bpp_tree_detail::construct_one(where, std::forward<in_value_t>(value), typename bpp_tree_detail::get_tag<iterator_t>::type());
		}

		template<class iterator_t>
		static void destroy_one_(iterator_t where)
		{
			bpp_tree_detail::destroy_one(where, typename bpp_tree_detail::get_tag<iterator_t>::type());
		}

		template<class iterator_t>
		static void destroy_range_(iterator_t destroy_begin, iterator_t destroy_end)
		{
			bpp_tree_detail::destroy_range(destroy_begin, destroy_end, typename bpp_tree_detail::get_tag<iterator_t>::type());
		}

		template<class iterator_from_t, class iterator_to_t>
		static void move_forward_(iterator_from_t move_begin, iterator_from_t move_end, iterator_to_t to_begin)
		{
			bpp_tree_detail::move_forward(move_begin, move_end, to_begin, typename bpp_tree_detail::get_tag<iterator_from_t>::type());
		}

		template<class iterator_from_t, class iterator_to_t>
		static void move_construct_(iterator_from_t move_begin, iterator_from_t move_end, iterator_to_t to_begin)
		{
			bpp_tree_detail::move_construct(move_begin, move_end, to_begin, typename bpp_tree_detail::get_tag<iterator_from_t>::type());
		}

		template<class iterator_t>
		static void move_next_to_and_construct_(iterator_t move_begin, iterator_t move_end, iterator_t to_begin)
		{
			bpp_tree_detail::move_next_to_and_construct(move_begin, move_end, to_begin, typename bpp_tree_detail::get_tag<iterator_t>::type());
		}

		template<class iterator_from_t, class iterator_to_t>
		static void move_and_destroy_(iterator_from_t move_begin, iterator_from_t move_end, iterator_to_t to_begin)
		{
			bpp_tree_detail::move_and_destroy(move_begin, move_end, to_begin, typename bpp_tree_detail::get_tag<iterator_from_t>::type());
		}

		template<class iterator_from_t, class iterator_to_t>
		static void move_construct_and_destroy_(iterator_from_t move_begin, iterator_from_t move_end, iterator_to_t to_begin)
		{
			bpp_tree_detail::move_construct_and_destroy(move_begin, move_end, to_begin, typename bpp_tree_detail::get_tag<iterator_from_t>::type());
		}

		template<class iterator_t, class in_value_t>
		static void move_next_and_insert_one_(iterator_t move_begin, iterator_t move_end, in_value_t&& value)
		{
			bpp_tree_detail::move_next_and_insert_one(move_begin, move_end, std::forward<in_value_t>(value), typename bpp_tree_detail::get_tag<iterator_t>::type());
		}

		template<class iterator_t>
		static void move_prev_and_destroy_one_(iterator_t move_begin, iterator_t move_end)
		{
			bpp_tree_detail::move_prev_and_destroy_one(move_begin, move_end, typename bpp_tree_detail::get_tag<iterator_t>::type());
		}
		...
		template<class in_value_t>
		pair_posi_t insert_first_(in_value_t&& value)
		{
		...
		template<class in_value_t>
		pair_posi_t insert_hint_(leaf_node_t* leaf_node, size_type where, in_value_t&& value)
		{
		...
		template<class in_value_t>
		pair_posi_t insert_pos_(leaf_node_t* leaf_node, size_type where, in_value_t&& value)
		{
			key_stack_t key_out;
		...
		template<bool is_leftish, class in_value_t>
		pair_posi_t insert_nohint_(in_value_t&& value)
		{
			if (root_.parent->size == 0)
			{
				return insert_first_(std::forward<in_value_t>(value));
		...
		template<class in_node_t>
		in_node_t* get_left_(in_node_t* node)
		{
			inner_node_t* parent;
		...
		template<class in_node_t>
		in_node_t* get_right_(in_node_t* node)
		{
			inner_node_t* parent;
			size_type where = get_parent_(node, parent);
			if (parent == nullptr)
		...
		template<class in_node_t>
		void get_left_right_parent_(inner_node_t* parent, size_type where, in_node_t*& left, inner_node_t*& left_parent, in_node_t*& right, inner_node_t*& right_parent)
		{
			if (parent == nullptr)
			{
				left = right = nullptr;
