...
namespace Ajy
{
	...
	template<class config_t> class size_balanced_tree;
	...
	protected:
		...
		{
			value_node_t(value_type const& v)
				: value(v)
			{
			}
			template<class ...args_t>
			value_node_t(args_t&& ...args)
		...
		struct root_node_t : public node_t, public key_compare, public node_allocator_t
		{
			...
			root_node_t(any_key_compare&& comp, any_allocator_t&& alloc)
				: key_compare(std::forward<any_key_compare>(comp)), node_allocator_t(std::forward<any_allocator_t>(alloc))
			{
			}
		};
		...
		struct head_t : public root_allocator_t
		{
			...
			head_t(any_allocator_t&& alloc)
				: root_allocator_t(std::forward<any_allocator_t>(alloc))
			{
			}
			root_node_t* root;
		};
	...
	public:
		...
		public:
			explicit iterator(node_t* in_node)
				: node(in_node)
			{
			}
		...
		public:
			explicit const_iterator(node_t* in_node)
				: node(in_node)
			{
			}
		...
		public:
			...
			typedef typename size_balanced_tree::pointer pointer;
		public:
			explicit reverse_iterator(node_t* in_node)
				: node(in_node)
			{
			}
		...
		public:
			...
			typedef typename size_balanced_tree::const_pointer const_pointer;
		public:
			explicit const_reverse_iterator(node_t* in_node)
				: node(in_node)
			{
			}
	...
	protected:
		//full
		template<class in_root_allocator_t, class in_node_allocator_t>
		size_balanced_tree(key_compare const& comp, in_root_allocator_t&& root_alloc, in_node_allocator_t&& node_alloc)
			: head_(std::forward<in_root_allocator_t>(root_alloc))
		{
	...
	public:
		//empty
		size_balanced_tree()
			: size_balanced_tree(key_compare(), allocator_type())
		{
		}
		//empty
		explicit size_balanced_tree(key_compare const& comp, allocator_type const& alloc = allocator_type())
			: size_balanced_tree(comp, alloc, alloc)
		{
		}
		//empty
		explicit size_balanced_tree(allocator_type const& alloc)
			: size_balanced_tree(key_compare(), alloc, alloc)
		{
		}
		//range
		template <class iterator_t>
		size_balanced_tree(iterator_t begin, iterator_t end, key_compare const& comp = key_compare(), allocator_type const& alloc = allocator_type())
			: size_balanced_tree(comp, alloc, alloc)
		{
			insert(begin, end);
		}
		//range
		template <class iterator_t>
		size_balanced_tree(iterator_t begin, iterator_t end, allocator_type const& alloc = allocator_type())
			: size_balanced_tree(key_compare(), alloc, alloc)
		{
			insert(begin, end);
		}
		...
		size_balanced_tree(size_balanced_tree const& other)
			: size_balanced_tree(other.get_comparator_(), other.get_root_allocator_(), other.get_node_allocator_())
		{
			sbt_copy_<std::false_type>(nullptr, other.get_root_());
		}
		//copy
		size_balanced_tree(size_balanced_tree const& other, allocator_type const& alloc)
			: size_balanced_tree(other.get_comparator_(), alloc, alloc)
		{
			sbt_copy_<std::false_type>(nullptr, other.get_root_());
		}
		//move
		size_balanced_tree(size_balanced_tree&& other)
			: size_balanced_tree(key_compare(), other.get_root_allocator_(), node_allocator_t())
		{
			std::swap(get_root_allocator_(), other.get_root_allocator_());
			std::swap(head_.root, other.head_.root);
		}
		//move
		size_balanced_tree(size_balanced_tree&& other, allocator_type const& alloc)
			: size_balanced_tree(key_compare(), alloc, alloc)
		{
			sbt_copy_<std::true_type>(nullptr, other.get_root_());
		}
		//initializer list
		size_balanced_tree(std::initializer_list<value_type> il, key_compare const& comp = key_compare(), allocator_type const& alloc = allocator_type())
			: size_balanced_tree(il.begin(), il.end(), comp, alloc)
		{
		}
		//initializer list
		size_balanced_tree(std::initializer_list<value_type> il, allocator_type const& alloc)
			: size_balanced_tree(il.begin(), il.end(), key_compare(), alloc)
		{
		}
		//destructor
		~size_balanced_tree()
		...
		typename std::enable_if<std::is_convertible<in_value_t, value_type>::value, iterator>::type insert(in_value_t&& value)
		{
			check_max_size_();
	...
	protected:
		...
		template<class ...args_t>
		node_t* sbt_create_node_(args_t&& ...args)
		{
